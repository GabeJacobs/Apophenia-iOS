...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
           selector:@selector(notInstalled:)
           name:@"SpotifyNotInstalled"
           object:nil];
   ...
   self.songTitle = [[UILabel alloc] initWithFrame:CGRectMake(self.albumArt.frame.origin.x + self.albumArt.frame.size.width + 7, self.albumArt.frame.origin.y + self.albumArt.frame.size.height/4, self.leftSideControls.frame.size.width - (self.albumArt.frame.origin.x + self.albumArt.frame.size.width + 3) - 8, 15)];
...
- (void)loadJson
{
    ...
    self.dataSource = [NSJSONSerialization JSONObjectWithData:data options:kNilOptions error:nil];
...
- (void)notInstalled:(NSNotification *)notification {
    [MBProgressHUD hideHUDForView:self.view animated:YES];
}
...
-(int)getVideoIndex{
    if(self.didLoadFirstVideo == NO){
        self.didLoadFirstVideo = YES;
        [self.arrayOfVideoIndexSelections removeObject:[NSNumber numberWithInt:self.currentVideoIndex]];
    }
...
- (void)switchDanceTapped {
    ...
    [FIRAnalytics logEventWithName:@"switched_dance" parameters:nil];
...
- (void)switchMusicTapped {
    ...
    [FIRAnalytics logEventWithName:@"switched_music" parameters:nil];
